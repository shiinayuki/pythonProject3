...
def usr_log_in():
    ...
    pwd = var_usr_pwd.get()
    tk.messagebox.showinfo(message='自动评教中！')
    # 用户名密码不能为空
    if name == '' or pwd == '':
        ...
        for url in hrefs:
            ...
            page_sum = get_num_from_string(
                web.find_element('xpath', '//*[@id="PagingControl1_divOuterClass"]/div/div[2]/span').text)
...
bt_login = tk.Button(window, text='自动评教', command=usr_log_in)
